...
function processSum(numbersList, callback) {
  let sum = numbersList.reduce(function(total, num) {
    return total += num;
  }, 0);
  return callback(sum);
...
function getFullNames(runners) {
  let fullName = [];
    runners.forEach(person => fullName.push(`${person.last_name}, ${person.first_name}`));
...
function firstNamesAllCaps(runners) {
  let firstName = [];
  runners.forEach(person => firstName.push(person.first_name.toUpperCase()));
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let sizes = runners.filter(function(person) {
    return person.shirt_size === tShirtSize;
  });
  return sizes.map(shirtSize => `${shirtSize.first_name} ${shirtSize.last_name}`);
}
...
function tallyUpDonations(runners) {
  let donations = runners.map(person => person.donation);
